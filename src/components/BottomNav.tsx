...
export function BottomNav({ badges }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 nav-glass pb-[env(safe-area-inset-bottom)]">
      <div className="flex items-center justify-around h-[56px] max-w-[480px] mx-auto">
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center justify-center gap-0.5 flex-1 h-full relative transition-colors duration-200 active:scale-90",
            ...
            >
              <div className="relative">
                ...
                {item.badge && item.badge > 0 && (
                  <span className="absolute -top-1 -left-1 min-w-[16px] h-4 rounded-full bg-destructive text-destructive-foreground text-[9px] font-bold flex items-center justify-center px-1 badge-pulse">
